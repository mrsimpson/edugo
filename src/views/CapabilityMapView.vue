<!--
  CapabilityMapView — the main capability map page (/catalog).
...
<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <!-- Page header -->
    <div class="mb-6">
      <h1 class="text-2xl font-bold text-gray-900">{{ t.catalog.pageTitle }}</h1>
      <p class="mt-1 text-sm text-gray-500">
        {{ t.catalog.resultCount(filteredNodes.length, nodes.length) }}
        <template v-if="filters.gap || filters.kmk.length > 0">
          {{ t.catalog.resultCountFiltered }}
        </template>
        <span
          aria-live="polite"
          aria-atomic="true"
          class="sr-only"
        >
          {{ t.catalog.resultLiveAnnouncement(filteredNodes.length) }}
        </span>
      </p>
    </div>
    ...
    <div class="flex flex-col lg:flex-row gap-6">
      ...
      <div class="flex-1">
        <div
          ...
          role="list"
          :aria-label="t.catalog.listAriaLabel"
        >
          <router-link
        ...
        <div
          ...
          aria-live="polite"
        >
          <p class="text-lg font-medium">{{ t.catalog.emptyHeading }}</p>
          <p class="text-sm mt-1">{{ t.catalog.emptyBody }}</p>
          <button
    ...
    >
      <div>
        <p class="text-sm font-medium text-blue-900">
          {{ t.catalog.gapCtaCount(filteredNodes.length) }}
        </p>
        <p class="text-sm text-blue-700 mt-0.5">
          {{ t.catalog.gapCtaBody }}
          <a
            :href="contributionUrl"
            target="_blank"
            rel="noopener noreferrer"
            class="underline font-medium"
          >{{ t.catalog.gapCtaLink }}</a>.
        </p>
      </div>
    </div>
  </div>
