 ...
 * - 1.5: User-visible text is German (status labels, domain names)
 */
<template>
  <article
    ...
    :aria-label="`Kompetenz: ${node.title}`"
  >
    <!-- Status badge + title -->
    ...
    <div class="mt-auto pt-2 border-t border-gray-100 flex items-center justify-between text-xs text-gray-500">
      <span v-if="(node.linkedEntryCount ?? 0) > 0">
        {{ node.linkedEntryCount }}
        {{ node.linkedEntryCount === 1 ? 'Tool' : 'Tools' }} eingetragen
      </span>
      <span v-else class="text-amber-600 font-medium">Noch kein Tool eingetragen</span>
      <span class="text-blue-600 font-medium">Details →</span>
    </div>
  </article>
</template>
